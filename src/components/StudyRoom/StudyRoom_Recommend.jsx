...
export default function StudyRoom_Recommend() {
    ...
    return (
      <>
        <div className="flex">
          ...
          <div className="w-3/4">
            <h2 className="text-xl font-bold mb-4">이런 멘토는 어떤가요?</h2>
  ...
  const Card = ({ name, mentorSkills, careerYear, rating,email }) => {
    const skills = Object.values(mentorSkills).map((skill) => skill.skill.skillName); // mentorSkills 객체의 skill 프로퍼티만 추출하여 배열 생성
  
    return (
      <div className="bg-white shadow-md p-4 rounded-lg">
        <h2 className="text-xl font-bold mb-2">이름 : {name}</h2>
        <p className="text-gray-600">
          Mentor Skills : {skills.join(", ")} {/* 스킬들을 쉼표로 구분하여 출력 */}
        </p>
